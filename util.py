...
def partition_dataset(rank, size, args):
    print('rank： {} ==> load train data'.format(rank))
    if args.dataset == 'cifar10':
        ...
        partition_sizes = [1.0 / size for _ in range(size)] # 每个元素都是 1.0 / size
        ...
        print('rank： {} ==> load test data'.format(rank))   ## 测试数据没有分区
    ...
    if args.dataset == 'cifar100':
        ...
        print('rank： {} ==> load test data'.format(rank))
        transform_test = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
        ])
